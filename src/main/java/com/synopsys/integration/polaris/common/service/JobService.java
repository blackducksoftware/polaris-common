...
public class JobService {
    ...
    public Optional<JobResource> getJobById(final String jobId) throws IntegrationException {
        final String uri = polarisHttpClient.getPolarisServerUrl() + JOBS_API_SPEC + "/" + jobId;
        return getJobByUrl(uri);
    }

    public Optional<JobResource> getJobByUrl(final String jobApiUrl) throws IntegrationException {
        final Request request = PolarisRequestFactory.createDefaultRequestBuilder().uri(jobApiUrl).build();
        return polarisService.getFirstResponse(request, JOB_RESOURCE.getType());
    }
